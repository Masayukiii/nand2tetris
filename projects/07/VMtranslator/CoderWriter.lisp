...
(defun arithmeticAssemblerList (line)
  (let ((label (string (gensym))))
  (cond
    ((string= (arg1 line) "add")(list "@R0" "M=M-1" "A=M" "D=M" "@R0" "M=M-1" "A=M" "M=D+M" "@R0" "M=M+1"))
    ((string= (arg1 line) "sub")(list "@R0" "M=M-1" "A=M" "D=M" "@R0" "M=M-1" "A=M" "M=D-M" "@R0" "M=M+1"))
    ((string= (arg1 line) "neg")(list "@R0" "M=M-1" "A=M" "M=-M" "@R0" "M=M+1"))
    ((string= (arg1 line) "eq" )(list "@R0" "M=M-1" "A=M" "D=M" "@R0" "M=M-1" "A=M" "D=M-D" "@eq" "D;JEQ" "@R0" "A=M" "M=0" (concatenate 'string "@" label) "0;JMP" "(eq)" "@R0" "A=M" "M=-1" (concatenate 'string "(" label ")") "@R0" "M=M+1"))
    ((string= (arg1 line) "gt" )(list "@R0" "M=M-1" "A=M" "D=M" "@R0" "M=M-1" "A=M" "D=M-D" "@gt" "D;JGT" "@R0" "A=M" "M=0" (concatenate 'string "@" label) "0;JMP" "(gt)" "@R0" "A=M" "M=-1" (concatenate 'string "(" label ")") "@R0" "M=M+1"))
    ((string= (arg1 line) "lt" )(list "@R0" "M=M-1" "A=M" "D=M" "@R0" "M=M-1" "A=M" "D=M-D" "@lt" "D;JLT" "@R0" "A=M" "M=0" (concatenate 'string "@" label) "0;JMP" "(lt)" "@R0" "A=M" "M=-1" (concatenate 'string "(" label ")") "@R0" "M=M+1"))
...
(defun writePush (segment index)
  (cond
   ((string= segment "constant")(pushAssemblerList index))
  )
)
(defun pushAssemblerList (index)
  (list (concatenate 'string "@" index) "D=A" "@R0" "A=M" "M=D" "@R0" "M=M+1")
)
